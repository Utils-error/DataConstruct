首次提交时，需要使用关联本地仓库与远程GitHub仓库

# 其他
 1.拉取远程主分支并允许合并不相关历史 git命令（master为分支名）
git pull origin master --allow-unrelated-histories

# 数组 
1. 定义：数组是由一组元素（值或变量）组成的数据结构。
2.  java中数组结构  
[      markword     ]   对象头，存储信息（8字节）  
[class指针] [数组大小]  指向的类对象指针、数组最大容量均为4字节  
[   值    ] [  值2   ]  具体值，int类型4个字节（举例）  
[   值3   ] [对齐字节]  java中对象均为8字节整数倍，不足使用对齐字节补足
3. 动态数组基本结构  
(1) 成员变量： 默认容量 当前大小 静态数组  
(2) 插入方法：当插入位置为末尾时，可以直接添加；非末尾位置时，需要将当前位置后续元素右移  
(3) 遍历方法：
    随机访问：直接获取指定索引位置元素；
    foreach：使用函数式接口，accept方法执行调用处具体的实现方法
    iterator迭代器遍历：需要实现iterable接口，并使用匿名内部类Iterator重写hasNext与Next方法，增强for循环底层使用的是迭代器遍历
    stream流：Stream.of()方法，将静态数组传入即可，注意可遍历范围  
(4) 删除方法：当删除位置为末尾时，直接删除；非末尾位置时，则需要将当前位置后续元素左移

# 二维数组
## 内存结构
 可以看成一个外层数组，存储每一个数组的引用，内层数组存储具体值，在空间分布上是连续的。
## 局部性原理
  在CPU读取内容数据时，会读取一个缓存行的数据，那么在较大的数组遍历时，按照先行后列的顺序遍历效率较高，可以充分利用缓存 
  而如果采取先列后行，那么每次都需要加载缓存行，当缓存达到上限后，旧缓存将被覆盖。

# 链表
## 内存结构
数据元素的线性集合，存储不连续，可以分类为：单向链表、双向链表、循环链表
## 随机访问性能
只能依次遍历，时间复杂度O(n)
for循环、whlie循环、iterator迭代器
## 插入与删除
起始位置：O(1);结束位置：已知尾节点O(1),未知仍为O(n);中间位置:O(n),因为需要找到插入位置
## 带哨兵的链表
避免因初始空链表而进行的空判断，简化代码

## 双向链表
由于在头尾各维护了一个指针，因此在进行操作时有两个地方需要注意：
(1) 在进行节点修改时，需要同时修改next和prev两个指针；
(2) 在进行尾部相关操作时，由于尾部节点已知可以进行快速操作，无须再遍历。

## 环形链表

# 递归
通过解决同类问题的更小子集作为解决方案，需要明确父问题与子问题的关系   
递归本质是隐式栈，需要在方法体中给出递归出口

## 尾调用与尾递归
a(){
// 其他代码
return b();
}
即最后一步操作为调用其他方法，如果是调用自己，那么就是尾递归。某些编译器(c/scara)  
会对这种代码做优化，可以同时调用a()与b(),使得a可以提前释放内存资源，而无须等待b。

## Master theorem主定理 计算递归时间复杂度
$T(n) = aT(\frac{n}{b})+f(n)$  
其中，a为子问题个数，b为子问题数据规模缩小倍数，具体计算结果为，考虑$x = log_b(a)$ ,$c$ 为 $f(n)$的中n的指数  
(1)当$x>c$,$T(n) =O(n^x)$;  
(2)当$c>x$,$T(n) =O(n^c)$;  
(3)当$x=c$,$T(n) =O(n^xlog n)$

当然，并不是所有的递归都可以由主定理计算得到，此时，就需要依据递归式进行推导。  
以斐波那契数列的递归实现为例，$T(n) = T(n-1) + T(n-2)$,那么就不符合主定理，因此需要依次递推.

## 常见链表算法
龟兔赛跑（Floyd判圈算法）判别链表中是否存在环   
递归求解  

## 常见排序算法
冒泡排序：左无序，右有序，左边比较相邻元素，使较大数向右移动，成为有序，O(n^2)
插入排序：左有序，右无序，遍历右侧元素插入到左侧有序序列中，O(n^2)
归并排序: 采用分治的思想，分：将一个无序数组不断拆分，直到子数组仅有一个元素（最小的有序表），治：将子数组按照顺序进行合并。  

# 队列
队列是以顺序方式维护的一组数据集合，从一端（尾）添加数据，从另一端（头）移除数据。  

## 实现方式  
(1) 基于链表实现(2)基于循环数组实现

当 被除数/除数，除数为2的n次幂时，其结果有显著规律：即被除数的后n位为余数，被除数右移n位为则为商  
那么如何获得被移除的余数呢，通过按位与求得：即余数mod = 被除数 & (2^n-1),商 = 被除数 >> n;  

这就要求必须有一个2的n次幂数，当入参不满足时，如何迅速的扩大到最近的2的n次幂数，可以使用扩散法。  

# 栈
线性数据结构，但只能在其一端进行输入数据的添加和移除，这一端称为栈顶，另一端为称为栈底。

# 双端队列
线性数据结构，可以在两端进行输入数据的添加和移除。

# 优先级队列
在普通队列的基础上，按照一定的优先级出队，并非先进先出  

# 堆  
一种基于树的数据结构，准确来说是基于完全二叉树实现。 这里需要简单说一下，二叉树是指每一个节点下都有两个字节点，  
而完全二叉树则是指除最后一层外，其余层全是满的，当然，如果最后一层是满的，也是完全二叉树，此外，每加入一个新节点时，都应该加入到最靠左侧的位置。

## 大顶堆与小顶堆
大顶堆与小顶堆的区别在:父节点的值与子节点值的关系,大顶则意味着父值大于子值，小顶则反之，最顶层的节点称为根节点。

## 使用线性结构（数组）存储非线性结构（堆）
(1) 数组  
**特征：从 索引0 存储数据(root)  对于任意节点i，其父节点索引位置为 floor((i-1)/2),左子节点为2i+1,右子节点为2i+2，其中floor为向下取整。  
**下沉与上浮：这其实是堆化的两种方式，当往堆中加入元素时，需要将新元素不断与其父节点进行比较，一直找到其应当存储的位置，此为上浮；  
当从堆中移除堆顶元素时，需要首先将堆顶与最后一个元素互换位置，这样可以快速移除，而后需要将被换到堆顶的元素与其子节点进行比较，一直找到其存储位置，此为下沉。

# 阻塞队列
背景：队列往往于应用消息队列消费场景中，那么当生产者、消费者操作队列时，如遇到队列满或空时，普通队列则会立刻拒绝当前操作，可用性较差。  
阻塞队列：  
（1）当队列满且需要继续向队列添加时，此时阻塞队列会采取“等待”的方式，直到队列有空余位置，同理，队列空时也会“等待”知直到有元素才会开放移除；  
（2）普通队列也存在着线程不安全的问题，因为操作队列并不是原子性操作，因此需要加锁实现。  
实现方案：  
（1）单锁实现；队列双端公用一个锁，并通过ReentrantLock与Condition的配合，实现“阻塞”，但此种方案会使得“头”“尾”两种类型操作互斥，这是不合理的。  
（2）双锁实现：队列头尾各用一个锁，这样“头”“尾”操作可以同时进行，此外，这里需要注意防止死锁，因此Condition需要再锁中进行。  
级联通知：  
在双锁实现的阻塞队列中，每添加/移除一个元素，都需要唤醒相应的poll()/offer()线程操作，由于唤醒也需要加锁，因此势必会影响到其他的正常操作，为尽可能地减少唤醒操作  
只有在队列为空时添加元素，才由offer()线程唤醒poll()线程，此后的poll()线程唤醒都由前一次的poll()线程

# 堆
堆的逻辑结构是一种二叉树，由最大堆和最小堆两种实现，其区别在于堆顶是最大值还是最小值，在进行数据操作时，有以下三个关键内容需要注意：  
(1) 堆化：即将一组数据调整为堆结构，其核心算法为：从其最后一个非叶子结点一直向上遍历到root节点，每一个节点都需要考虑下沉，直到下沉到最下层。  
(2) 下沉：某一节点需要和其左右子节点进行比较，如符合规则则进行交换，一直下沉到最底层。  
(3) 上浮：某一节点需要和父节点进行比较，如符合规则则进行交换，一直上浮到最顶层。  
在堆中数据初始化往往需要使用“堆化”，添加数据(到末尾)时，需要使用到上浮，删除数据时，需要先将末尾数据交换到顶层，删除末尾数据的同时并将顶层的数据进行下沉。 
java有相应的实现，PriorityQueue,可以通过构造方法中参数指定最大堆与最小堆。
